...
let
  ...
  defaultWeechatScriptOverrides = {
    weetris = attrs: {};
    slack = attrs: {
      nativeBuildInputs = (attrs.nativeBuildInputs or []) ++ [pkgs.python3Packages.wrapPython];
    ...
    otr = attrs: {
      nativeBuildInputs = (attrs.nativeBuildInputs or []) ++ [pkgs.python3Packages.wrapPython];
      pythonPath = with pkgs.python3.pkgs; [ (potr.overridePythonAttrs (oldAttrs: {
  ...
  scripts = pkgs.callPackage ./scripts.nix { inherit defaultWeechatScriptOverrides; };
  pkg = pkgs.weechat-unwrapped.overrideAttrs (old: { patches = [ ./upgrade.patch ]; });
  configuredPkg = pkgs.wrapWeechat pkg {
